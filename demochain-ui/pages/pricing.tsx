...
const faqs = [
  ...
  {
    q: '支持哪些付款方式？',
    a: '支持 PayPal、信用卡、借记卡等国际主流支付方式。',
  },
  {
    q: '可以申请退款吗？',
    a: '所有付费计划支持 7 天无理由退款。如有任何问题，请通过邮件联系我们。',
  },
  {
    q: '提供技术支持吗？',
    a: '我们不提供技术支持服务。平台设计简单易用，如遇到使用问题，建议查看演示说明或重新尝试操作。',
  },
  {
    q: '会有新功能更新吗？',
    a: '是的，我们会持续更新和改进平台功能。所有付费会员都能免费享受功能更新，无需额外付费。',
  },
];
  ...
  return (
      <div className="px-4 lg:px-12 max-w-screen-2xl mx-auto py-10 md:py-14">
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6 mb-12">
          {plans.map((p, i) => (
            <div key={i} className={`rounded-2xl border bg-white/80 backdrop-blur p-5 dark:bg-[#15161a]/80 dark:border-[#2a2c31] ${i === 2 ? 'ring-2 ring-orange-500 dark:ring-orange-400' : ''}`}>
              ...
              <button className={`w-full rounded-xl py-2.5 hover:opacity-90 ${i === 0 ? 'bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200' : 'bg-gradient-to-r from-orange-500 to-purple-600 text-white'}`}>
                {p.cta}
              </button>
            </div>
          ))}
