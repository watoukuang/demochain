                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            {validators.map(v => {
                                ...
                                return (
                                    <tr key={v.id}
                                        className={`border-b border-gray-100 dark:border-gray-700 ${isSelected ? 'bg-green-50 dark:bg-green-900/20' : ''}`}>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center space-x-2">
                                                <div
                                                    className={`w-3 h-3 rounded-full ${isSelected ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}></div>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center space-x-2">
                                                <div
                                                    className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2 max-w-20">
                                                    <div
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                                <button
                                    onClick={pick}
                                    disabled={total <= 0 || validators.length === 0}
                                    className="px-6 py-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 text-white font-medium hover:from-blue-600 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    🎲 随机选择出块者
                                </button>
                                <div className="text-sm text-gray-600 dark:text-gray-400">
                                    总质押: <span
                                    className="font-medium text-gray-900 dark:text-white">{total.toLocaleString()}</span>
                                </div>
                            </div>
                            {selected && (
