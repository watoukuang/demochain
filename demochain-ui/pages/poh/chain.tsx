...
export default function PohChain(): React.ReactElement {
  const [difficulty, setDifficulty] = useState(2)
  const [blocks, setBlocks] = useState<MiniBlock[]>([{
    height: 1, nonce: 1, data: 'POH 创世', previous:'0'.repeat(64), timestamp: Date.now(), hash:'0'.repeat(64), award:'0', miner:'Genesis', txs: []
  }])
  ...
  const add = () => {
    ...
    setBlocks(b=>[...b,{height:b.length+1, nonce:n, data:'POH 产块(按时序)', previous:last.hash, timestamp:ts, hash:h, award:'0', miner:'POH', txs: []}])
  }
  return (
    <div className="px-4 py-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="p-4 rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">POH · 区块链</h1>
