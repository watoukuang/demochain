    ...
    <div className="relative" ref={menuRef}>
      ...
      {menuOpen && (
        <div
          role="menu"
          aria-label="主题切换"
          className="absolute right-0 mt-2 w-56 rounded-2xl border bg-white/98 backdrop-blur-sm shadow-lg ring-1 ring-black/5 p-2 border-gray-200 dark:bg-[#1e1e1e] dark:border-[#2d2d30] dark:ring-white/5 dark:text-gray-200 z-50"
        >
          {themeOptions.map((option, index) => (
            <React.Fragment key={option.key}>
              ...
              >
                <span className={option.key === 'light' ? 'text-yellow-500' : option.key === 'system' ? 'text-indigo-600' : 'text-gray-700 dark:text-gray-300'}>
                  {option.icon}
                </span>
